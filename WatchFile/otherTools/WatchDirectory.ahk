...
#Persistent
OnExit,Exit
WatchDirectory("D:\Program Files\Epic Games\EvolandLegendaryEditdWwMI",0)
SetTimer,WatchFolder,100
Return
WatchFolder:
    WatchDirectory("RegisterChanges")
...
WatchDirectory(WatchFolder="", WatchSubDirs=true)
{
    ...
    If !(WatchFolder){
        ...
        Loop {
            ;MODIFIED
            pNext := NumGet( PointerFNI + 0,"uint" )
            Action := NumGet( PointerFNI + 4 ,"uint")
            FileNameLen := NumGet( PointerFNI + 8,"uint" )
            ; pFileName := ( PointerFNI + 12 )
            ; VarSetCapacity(lpFilename, FileNameLen, 0)
            pFileName := StrGet( PointerFNI + 12,FileNameLen/2)
            If (Action < 0x6){
                ; VarSetCapacity( FileNameANSI, FileNameLen )
                ; DllCall( "WideCharToMultiByte",UInt,0,UInt,0,UInt,pFileName,UInt,-1,Str,FileNameANSI,UInt,FileNameLen+1,UInt,0,UInt,0)
                ; %WatchFolder%(Action,CurrentFolder,SubStr( FileNameANSI, 1, FileNameLen/2 ))
                ;MODIFIED
                %WatchFolder%(Action,CurrentFolder,pFileName)
            }
